...
async function 执行百度搜索(查询词) {
  ...
  if (!百度密钥) {
    ...
    return '【联网搜索失败】百度搜索密钥未配置。请在设置中配置百度搜索密钥后重试。';
  }
  ...
  try {
    ...
    if (!数据) {
      ...
      try {
        // 尝试本地代理（localhost:3778，由 baidu-proxy.js 启动）
        const 代理响应 = await fetch('http://localhost:3778/proxy/baidu-search', {
          method: 'POST',
          headers: {
        ...
        if (代理响应.ok) {
          数据 = await 代理响应.json();
          console.log('通过本地代理搜索成功');
        } else {
          console.warn('本地代理请求失败:', 代理响应.status);
        }
      } catch (代理错误) {
        console.warn('本地代理不可用，降级到直连 fetch:', 代理错误.message);
      }
      
      if (!数据) {
        // 直连 fetch（浏览器可能因 CORS 失败，但某些场景可通）
        const 直连URL = 'https://qianfan.baidubce.com/v2/ai_search/web_search';
        try {
          const 响应 = await fetch(直连URL, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': 'Bearer ' + 百度密钥
          ...
          if (!响应.ok) {
            ...
            console.warn('百度搜索直连失败:', 响应.status, 错误文本);
            return `【联网搜索失败】百度API请求失败 (HTTP ${响应.status})。请尝试启动本地代理：node baidu-proxy.js${错误文本.slice(0, 100)}`;
          }
          数据 = await 响应.json();
        } catch (直连错误) {
          console.warn('百度搜索直连失败:', 直连错误.message);
          return '【联网搜索失败】浏览器无法直接请求百度 API（CORS 限制）。请确保已启动本地代理：node baidu-proxy.js，或切换到手机端使用。';
        }
      }
    }
    ...
    if (数据.code) {
      const 错误信息 = 数据.message || 数据.code;
      console.warn('百度搜索返回错误:', 错误信息);
      return `【联网搜索失败】百度API返回错误: ${错误信息}`;
    }
...
**请严格按照以上搜索结果回答用户的问题。这是实时联网搜索结果，不是你的训练数据。如果搜索结果为空或不足以回答，请如实说缺乏足够信息。**`;
    ...
    return `【联网搜索失败】搜索请求异常: ${错误.message || 错误}`;
  }
}
...
async function 调用API(消息列表, 调用轮次 = 0, 状态回调 = null, 最大轮次覆盖 = null, 工具调用历史 = []) {
  ...
  const _timeoutId = setTimeout(() => _abortCtrl.abort(), 120000); // 120秒超时

  if (状态回调) 状态回调(调用轮次 === 0 ? `💭 ${助手名}正在理解消息...` : `💭 ${助手名}继续思考...`);

  let 响应;
  try {
    响应 = await fetch('https://api.deepseek.com/chat/completions', {
      method: 'POST',
    ...
    if (e.name === 'AbortError') throw new Error('API请求超时（120秒），请检查网络后重试');
    throw e;
  }
  clearTimeout(_timeoutId);
...
async function 调用API流式(消息列表, 每块回调) {
  ...
  // 流式 AbortController
  const _streamAbortCtrl = new AbortController();
  const _streamTimeoutId = setTimeout(() => _streamAbortCtrl.abort(), 300000); // 流式5分钟超时
  ...
  try {
    响应 = await fetch('https://api.deepseek.com/chat/completions', {
      ...
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${密钥}` },
      body: JSON.stringify(请求体),
      signal: _streamAbortCtrl.signal
    });
  } catch (e) {
    clearTimeout(_streamTimeoutId);
    if (e.name === 'AbortError') throw new Error('流式请求超时（5分钟），请检查网络后重试');
    throw e;
  }
  clearTimeout(_streamTimeoutId);
  if (!响应.ok) throw new Error(`API 请求失败 (${响应.status})`);
...
async function 处理备忘录工具(工具名, 参数) {
  ...
  if (当前权限缓存 !== null && 当前权限缓存.length === 0) {
    if (工具名 !== '创建备忘录') {
      return '你没有权限操作。如需操作，请向用户申请开放权限。';
    }
  }
