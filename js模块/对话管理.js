...
你可以在回复中用方括号指定表情和动作，让角色做出对应反应。
格式: [emo:标签] 表情  [mot:标签] 动作

${tags}

规则:
- 标记放句首或句尾，无特别需要可不加
- 标有(持续)的表情会持续到下一轮对话，适合表达持续情绪
- 标有(~Ns)的动作需要约N秒播放，避免同时触发多个长动作
- 每轮回复最多加1个表情+1个动作，不要滥用
- 用户要求你做特定表情时，用对应的标签配合符合人设的回话`);
      }
    }
  } catch (e) {
